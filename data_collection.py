    ...
    for action, sequence, frame in product(actions, range(sequences), range(frames)):
        ...
        if frame == 0:
            while True:
                ...
                cv2.putText(image, 'Record Dataset "{}". Sequence number {}.'.format(action, sequence),
                ...
                cv2.putText(image, 'Tekan "Space Bar" untuk Capture.', (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
